			...
			try {
                ...
                if (reply && reply.ok && typeof reply.data === 'string') {
                    ...
                    for (let i = 1; i < parts.length; i++) {
                        try { await message.channel.send(parts[i]); } catch (e) { console.error('send followup chunk error:', e?.name, e?.message, e?.stack); }
                    }
                } else {
                    await message.reply('Error: ' + (reply && reply.error ? (reply.error.message || JSON.stringify(reply.error)) : 'AI error'));
                }
            } catch (e) {
                console.error('mention->AI error:', e?.name, e?.message, e?.stack);
                try { await message.reply('Error processing your request'); } catch {}
            }
		}
        } catch (err) {
            console.error('messageCreate handler error:', err?.name, err?.message, err?.stack);
        }
	});
};
