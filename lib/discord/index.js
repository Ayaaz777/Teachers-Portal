...
async function start() {
    ...
    const onReady = async () => {
        ...
        try {
            ...
            const rawDraft = normalizeName(process.env.DISCORD_DRAFT_HOLDING_CHANNEL || 'mod-drafts');
            ...
            for (const want of allowNames) {
                const list = nameToChannels.get(want) || [];
                // prefer a text-based channel when available
                let chosen = list.find(c => c.isTextBased && c.isTextBased()) || list[0] || null;
                if (chosen) {
                    aiTools.WRITE_ALLOWLIST.add(String(chosen.id));
                    resolved.push({ name: want, id: chosen.id });
                } else {
                    missing.push(want);
                }
            }
