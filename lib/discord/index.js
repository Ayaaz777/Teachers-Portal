    ...
    try {
        ...
        if (!resolvedGuildId) {
            ...
            if (GUILD_ID) {
                ...
                console.error('It looks like your DISCORD_GUILD_ID contains extra characters (URL or query string). Value seen:', JSON.stringify(GUILD_ID));
            }
            return;
        }
        ...
        try {
            ...
            try {
                ...
                if (code === 10004 || fetchErr && String(fetchErr).toLowerCase().includes('unknown guild') || fetchErr && fetchErr.status === 404) {
                    ...
                    console.error('Fixes:');
                    console.error('- Invite the bot to the server using the OAuth invite URL below (replace CLIENT_ID if needed):');
...
async function start() {
    ...
    // Listen to both 'ready' and 'clientReady' for forward-compat with discord.js v15
