...
function resolveChannelFromInput(client, input) {
    // Accept numeric id, mention like <#123>, or a pasted invite/URL. Return a channel object or null.
    if (!input) return null;
    const s = String(input).trim();
    // raw numeric snowflake
    const idMatch = s.match(/^(?:<#)?(\d{17,20})(?:>)?$/);
    if (idMatch) {
        const id = idMatch[1];
        try {
    ...
    // try to parse a discord.gg invite or url-like content for guild or channel id
    const urlMatch = s.match(/(\d{17,20})/);
    if (urlMatch) {
        const id = urlMatch[1];
        return client.channels.cache.get(id) || null;
    }
...
function buildToolDefs(client, opts = {}) {
    ...
    defs.push({
        ...
        description: 'Send a DM to a user. DRAFT-ONLY: all DMs must be reviewed by a human first. INPUT: { userId: string, content: string }',
        input_schema: { type: 'object', properties: { userId: { type: 'string' }, content: { type: 'string' } }, required: ['userId','content'] },
    });
    ...
    try {
        switch (name) {
            case 'discord_list_channels': {
                ...
                return { ok: true, data: channels.map(ch => ({ type: 'text', text: `${ch.name} (id: ${ch.id})` })), isError: false, ms: 0 };
            }
            ...
            case 'discord_search_messages': {
                ...
                const limit = Math.min(200, Number(input.limit) || 200);
                const msgs = await channel.messages.fetch({ limit });
                const hits = msgs.filter(m => (m.content || '').toLowerCase().includes(query)).map(m => `${m.author.username}: ${m.content}`).reverse();
            ...
            case 'discord_get_user': {
                ...
                return { ok: true, data: [{ type: 'text', text: `user: ${user.username}#${user.discriminator || '0000'} id:${user.id}` }], isError: false, ms: 0 };
            }
            ...
            case 'discord_send_message': {
                ...
                if (!WRITE_ALLOWLIST.has(channel.id)) {
                    ...
                    // create draft: send to configured mod-drafts channel if available
                    const draftChannel = client.channels.cache.find(c => c.name === DRAFT_HOLDING_CHANNELS.modDrafts) || null;
                    if (draftChannel) {
                        await draftChannel.send(`DRAFT for ${channel.name} (${channel.id}):\n` + chunks[0]);
                    }
                ...
                return { ok: true, data: [{ type: 'text', text: `SENT message_id:${sent.id}` }], isError: false, ms: 0 };
            }

            case 'discord_send_dm': {
                // Always draft-only: do not send DMs directly
                const userId = input.userId;
                const content = String(input.content || '').trim();
                if (!userId || !content) return { ok: false, data: [{ type: 'text', text: 'userId and content required' }], isError: true, ms: 0 };
                const safeContent = sanitizeForDiscord(content);
                const draftChannel = client.channels.cache.find(c => c.name === DRAFT_HOLDING_CHANNELS.modDrafts) || null;
                if (draftChannel) {
                    await draftChannel.send(`DM DRAFT for user ${userId}:\n` + safeContent.slice(0, 1800));
                }
                return { ok: true, data: [{ type: 'text', text: 'DRAFT_CREATED' }], isError: false, ms: 0 };
            }
            ...
            case 'discord_create_thread': {
                ...
                if (input.messageId) {
                    ...
                    return { ok: true, data: [{ type: 'text', text: `THREAD_CREATED id:${thread.id}` }], isError: false, ms: 0 };
                }
