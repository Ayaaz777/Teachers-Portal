...
function resolveChannelFromInput(client, input) {
    ...
    if (!input) {
        const resolved = null;
        console.log('[discord-tool] resolveChannel input:', input, 'resolved:', resolved && resolved.name, 'id:', resolved && resolved.id);
        return resolved;
    }
    ...
    if (idMatch) {
        const id = idMatch[1];
        try {
            const resolved = client.channels.cache.get(id) || null;
            console.log('[discord-tool] resolveChannel input:', input, 'resolved:', resolved && resolved.name, 'id:', resolved && resolved.id);
            return resolved;
        } catch (e) {
            console.log('[discord-tool] resolveChannel input:', input, 'resolved:', null, 'id:', null);
            return null;
        }
    }
        ...
        switch (name) {
            ...
            case 'discord_search_messages': {
                try {
                    ...
                    const channelNames = [channel.name || 'unknown'];
                    console.log('[discord-tool] search query:', query, 'channels searched:', channelNames.join(','), 'matches found:', hits.length);
