...
class AiChatService {
	...
	_buildClaudeTools() {
		...
		// If discord client/tools are available, register them as well. We require lazily
		// so this module can be loaded without Discord runtime present.
		try {
			const discordTools = require('./discord/ai-tools');
			if (discordTools && typeof discordTools.buildToolDefs === 'function' && global.__discord_client) {
				const defs = discordTools.buildToolDefs(global.__discord_client);
				if (Array.isArray(defs)) for (const d of defs) tools.push(d);
				console.log('[ai-chat] Registered discord_* Claude tools:', defs.map(d => d.name).join(', '));
			}
		} catch (e) {
