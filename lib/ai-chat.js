...
class AiChatService {
	...
	_buildClaudeTools() {
		...
		try {
			...
			// not fatal; discord tools optional at startup
		}
	...
	async _callNotionTool(toolName, args) {
		...
		if (typeof toolName === 'string' && toolName.startsWith('discord_')) {
			...
			if (discordTools && typeof discordTools.callTool === 'function') {
				...
				const result = await discordTools.callTool(global.__discord_client || null, toolName, args || {});
	...
	_systemPrompt() {
		return String.raw`YOUR CAPABILITIES
...
BLUEPRINT REMINDERS (override WRITE_ALLOWLIST)

These topics force draft-only regardless of which channel is targeted:
- Money — any mention of payments, payroll, salary, refunds, invoices, fees.
- Outbound emails — any message addressed to teachers or schools.
- Contracts — anything that modifies or references a school contract.
- Rejection / firing / dispute — sensitive personnel decisions.

For these, override allowlist behaviour and force draft to #mod-drafts even if the target was admin-scratchpad. The rationale field must say "Blueprint guard: <category>".
