...
async function ensureWhisperServer() {
  ...
  startPromise = (async () => {
    ...
    if (!voiceFileExists(cfg.model)) {
      console.error(
        `[whisper-server] Model not found: ${cfg.model}`,
      );
      console.error('[whisper] Download: https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-small.en.bin');
      console.error('[whisper] Place at:', cfg.model);
      return false;
    }
