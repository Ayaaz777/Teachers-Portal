...
async function getSummary({ userEmail, weekStart }) {
	try {
...
async function getSummariesInRange({ userEmail, startDate, endDate }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE" } };
		const { data, error } = await sb.from("voice_weekly_summaries")
...
async function storeSummary({ userEmail, weekStart, weekEnd, summaryText, topicTags, turnCount }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE" } };
...
async function fetchConversationsInRange({ userEmail, startDate, endDate }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE" } };
...
async function searchConversationsInRange({ userEmail, startDate, endDate, searchTerms }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE" } };
