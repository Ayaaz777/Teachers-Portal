...
async function storePageRef({ userEmail, pageId, pageName, databaseId, sourceCid }) {
	try {
		...
		const { error } = await sb.from("voice_page_refs").upsert({
			...
			onConflict: "user_email, page_name",
...
async function findPageRef({ userEmail, pageName }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE", message: "Supabase not configured" } };
		const normalized = pageName.trim().toLowerCase().replace(/\s+/g, " ");
...
async function listPageRefs({ userEmail }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE", message: "Supabase not configured" } };
		const { data, error } = await sb.from("voice_page_refs")
			.select("page_id, page_name, database_id, created_at, updated_at, access_count, last_accessed_at")
			.eq("user_email", userEmail)
...
async function removePageRef({ userEmail, pageName }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE", message: "Supabase not configured" } };
		const normalized = pageName.trim().toLowerCase().replace(/\s+/g, " ");
...
async function searchPageRefs({ userEmail, search, limit = 5 }) {
	try {
		const sb = getAdminClient();
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE", message: "Supabase not configured" } };
...
async function bumpAccess({ userEmail, pageName }) {
	try {
		const sb = getAdminClient();
		if (!sb) return;
		const normalized = pageName.trim().toLowerCase().replace(/\s+/g, " ");
