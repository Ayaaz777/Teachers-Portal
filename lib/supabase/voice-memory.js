...
async function storeFact({ userEmail, key, value, sourceCid, confidence, userName, speakerId }) {
	try {
		...
		let upsertErr = null;
		// Use upsert so duplicate facts update instead of erroring
		const res = await sb.from("voice_facts").upsert(payload, { onConflict: 'user_email,fact_key' });
...
async function listFacts({ userEmail, userName, speakerId }) {
	try {
		...
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE", message: "Supabase not configured" } };
		const sId = speakerId || userName || 'unknown';
		let query = sb.from("voice_facts").select("fact_key, fact_value, confidence, access_count, last_accessed_at, created_at, updated_at, speaker_id").eq("user_email", userEmail).eq("speaker_id", sId).order("updated_at", { ascending: false });
...
async function getRecentConversations({ userEmail, userName, speakerId, limit = 100 }) {
	try {
		...
		if (!sb) return { ok: false, error: { code: "SUPABASE_UNAVAILABLE", message: "Supabase not configured" } };
		const sId = speakerId || userName || 'unknown';
		let query = sb.from("voice_conversations").select("turn_role, content, user_name, speaker_id").eq("user_email", userEmail).eq("speaker_id", sId).order("created_at", { ascending: false }).limit(limit);
		const { data, error } = await query;
		if (error) {
...
async function searchFacts({ userEmail, search, limit = 5, userName, speakerId }) {
	try {
		...
		const sId = speakerId || userName || 'unknown';
		let query = sb.from("voice_facts").select("fact_key, fact_value, confidence, access_count, last_accessed_at, created_at, updated_at, speaker_id").eq("user_email", userEmail).eq("speaker_id", sId).or(`fact_key.ilike.${pattern},fact_value.ilike.${pattern}`).order("updated_at", { ascending: false }).limit(limit);
		const { data, error } = await query;
		if (error) {
...
async function bumpAccess({ userEmail, key, userName, speakerId }) {
	try {
		...
		if (!sb) return;
		const sId = speakerId || userName || 'unknown';
		let query = sb.from("voice_facts").update({ last_accessed_at: new Date().toISOString() }).eq("user_email", userEmail).eq("fact_key", key).eq("speaker_id", sId);
		await query;
	} catch {}
}
