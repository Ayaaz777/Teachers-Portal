...
async function retrieve({ userEmail, query, k, confidenceThreshold, staleDays }) {
	k = k || 10;
	...
	if (keywords.length > 0) {
		const [kwFactRes, kwRefRes] = await Promise.all([
			Promise.all(keywords.map(kw => voiceMemory.searchFacts({ userEmail, search: kw, limit: 3 }))),
			Promise.all(keywords.map(kw => pageMemory.searchPageRefs({ userEmail, search: kw, limit: 3 }))),
		]);
		const fMap = new Map();
	...
	if (query && (totalRecent + kwFacts.length + kwRefs.length) < k * 2) {
		const semResult = await voiceMemory.recallSemantic({ userEmail, queryText: query, k: 5 });
		...
		if (tr) {
			...
			if (existing.ok && Array.isArray(existing.data) && existing.data.length > 0) {
				...
				const generated = await weeklySummaries.generateSummary({
					userEmail, weekStart: tr.startDate, weekEnd: tr.endDate,
				});
	...
	for (const f of dedupedFacts) {
		if (f.fact_key) voiceMemory.bumpAccess({ userEmail, key: f.fact_key }).catch(() => {});
	}
	for (const r of scoredRefs) {
		if (r.page_name) pageMemory.bumpAccess({ userEmail, pageName: r.page_name }).catch(() => {});
	}
