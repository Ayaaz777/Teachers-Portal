...
function getPoolConfig() {
  const poolSize = +(process.env.RME_CHATTERBOX_POOL_SIZE || '1');
...
function createChatterboxPool(opts = {}) {
    ...
    _readyPromise = (async () => {
      ...
      healthTimer = setInterval(() => {
        for (const entry of backends.values()) {
          ...
          if (entry.inFlight > 0) continue; // active synth proves port is alive
  ...
  async function acquire() {
    ...
    throw new Error('chatterbox-pool: no healthy backends after 30s');
  }
