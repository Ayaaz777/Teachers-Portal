	...
	try {
		...
		try {
			for await (const pcmChunk of masterStream) {
				...
				while (acc.length >= BYTES_PER_SEC) {
					const pcm = acc.subarray(0, BYTES_PER_SEC);
					acc = acc.subarray(BYTES_PER_SEC);
					const wav = Buffer.concat([buildWavHeader(BYTES_PER_SEC, OUTPUT_SR, 1, 16), pcm]);
					if (typeof opts.onChunk === "function") {
						opts.onChunk({ index: chunkIndex, audio: wav, mimeType: "audio/wav", durationMs: 1000 });
					}
					chunkIndex++;
