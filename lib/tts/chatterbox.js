...
async function ensureServer() {
	...
	try {
		...
		/* Kill leftover Python process on port 8123 before spawning fresh */
		try {
		...
		serverProcess = spawn(pythonCmd, [scriptPath, "--model", "original"], {
...
async function synthesizeChunk(text, opts) {
	const cfg = getConfig();
	// Ensure text is stripped of markdown/prosody meta before sending to Chatterbox
...
function shutdown() {
	if (serverProcess) {
		console.log("[chatterbox] Killing TTS server process...");
		serverProcess.kill();
		serverProcess = null;
		console.log("[chatterbox] TTS server killed.");
	}
	_ready = false;
}
