...
async function synthesizeStreaming(opts) {
	...
	try {
		...
		nodeInput.on("error", (err) => {
			console.error(`[chatterbox-stream] port=${port} pcm-stream error:`, err.message);
		});
		let masterStream;
		try {
			masterStream = streamThroughMaster({
				inputStream: nodeInput,
				inputSampleRate: 24000,
				inputBitDepth: 16,
				signal: opts?.signal,
			});
		} catch (streamErr) {
			const errPort = port || (baseUrl ? new URL(baseUrl).port : '?');
			console.error(`[chatterbox-stream] port=${errPort} streamThroughMaster failed:`, streamErr.message);
			throw streamErr;
		}
		const OUTPUT_SR = 48000;
		const BYTES_PER_SEC = OUTPUT_SR * 2;
		const CHUNK_DUR_MS = 250;
		const CHUNK_BYTES = Math.round(BYTES_PER_SEC * (CHUNK_DUR_MS / 1000));
		let acc = Buffer.alloc(0);
		let chunkIndex = 0;
		let totalDurationMs = 0;
		try {
			for await (const pcmChunk of masterStream) {
				acc = Buffer.concat([acc, pcmChunk]);
				while (acc.length >= CHUNK_BYTES) {
					const pcm = acc.subarray(0, CHUNK_BYTES);
					acc = acc.subarray(CHUNK_BYTES);
					const wav = Buffer.concat([buildWavHeader(CHUNK_BYTES, OUTPUT_SR, 1, 16), pcm]);
					if (typeof opts.onChunk === "function") {
						opts.onChunk({ index: chunkIndex, audio: wav, mimeType: "audio/wav", durationMs: CHUNK_DUR_MS });
					}
					chunkIndex++;
					totalDurationMs += CHUNK_DUR_MS;
				}
			}
		} catch (streamErr) {
			const errPort = port || (baseUrl ? new URL(baseUrl).port : '?');
			console.error(`[chatterbox-stream] port=${errPort} for-await failed:`, streamErr.message);
			throw streamErr;
		}
		if (acc.length > 0) {
			const partialMs = Math.round((acc.length / BYTES_PER_SEC) * 1000);
			const wav = Buffer.concat([buildWavHeader(acc.length, OUTPUT_SR, 1, 16), acc]);
			if (typeof opts.onChunk === "function") {
				opts.onChunk({ index: chunkIndex, audio: wav, mimeType: "audio/wav", durationMs: partialMs });
			}
			chunkIndex++;
			totalDurationMs += partialMs;
		}
		const totalMs = Date.now() - t0;
		const logPort = port || '?';
		console.log(`[chatterbox] streaming-synth port=${logPort} totalMs=${totalMs} chars=${payloadText.length} chunks=${chunkIndex} audioMs=${totalDurationMs}`);
		return {
			ok: true,
