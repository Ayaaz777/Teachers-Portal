...
function getConfig() {
	...
	const voice = String(process.env.RME_CHATTERBOX_VOICE || "abigail").trim() || "abigail";
	const exaggeration = parseFloat(process.env.RME_CHATTERBOX_EXAGGERATION) || 0.35;
	const cfgWeight = parseFloat(process.env.RME_CHATTERBOX_CFG_WEIGHT) || 0.55;
	return { serverUrl, voice, exaggeration, cfgWeight };
}
...
async function ensureServer() {
	...
	try {
		...
		console.warn("[chatterbox] Server not ready after 60s");
...
async function synthesizeChunk(text, opts) {
	const cfg = getConfig();
	const res = await fetch(`${cfg.serverUrl}/v1/audio/speech`, {
		method: "POST",
...
async function synthesize(opts) {
	const normalized = normalizeForTts(String(opts?.text || "").trim());
	...
	await acquireSlot();
	try {
		let wavBuffer = await synthesizeChunk(fullText, opts);
		const totalMs = Date.now() - t0;
		const inputSampleRate = readWavSampleRate(wavBuffer);
		wavBuffer = await processWavBuffer(wavBuffer, inputSampleRate);
		console.log(`[chatterbox] synth totalMs=${totalMs} chars=${fullText.length} bytes=${wavBuffer.length}`);
...
function mergeWavBuffers(buffers) {
	...
	const frameSize = channels * BYTES_PER_SAMPLE;
	let totalFrames = 0;
	const dataChunks = [];
	...
	const header = buildWavHeader(totalBytes, sampleRate, channels);
...
function buildWavHeader(dataSize, sampleRate, channels) {
	channels = channels || 1;
	const frameSize = channels * BYTES_PER_SAMPLE;
...
function wavDurationMs(buf) {
	...
	return (dataSize / frameSize / sampleRate) * 1000;
}
