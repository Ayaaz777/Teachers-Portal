...
async function synthesizeStreaming(opts) {
	...
	try {
		...
		try {
			...
			console.error(`[chatterbox-stream] port=${port} streamThroughMaster failed:`, streamErr.message);
			throw streamErr;
		}
		...
		try {
			...
			console.error(`[chatterbox-stream] port=${port} for-await failed:`, streamErr.message);
			throw streamErr;
		}
		if (acc.length > 0) {
